...
class AuthController {
    ...
    try {
        ...
        // ✅ Redirect to the home page after successful activation (login is a modal, not a route)
        res.redirect(`${AppConfig.frontend_Url}/?activated=true`);

    } catch (exception) {
        // ✅ Redirect to the home page if activation fails
        if (exception.status === "ACTIVATION_TOKEN_NOT_FOUND" || exception.status === "ACTIVATION_TOKEN_MISSING") {
            return res.redirect(`${AppConfig.frontend_Url}/?activated=false`);
        }
        next(exception)
    }
}
